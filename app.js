    ...
    function (session) {
        ...
        if (isChannelConversation) {
            ...
            msg.attachments([
                new builder.HeroCard(session)
                    .buttons([
                        builder.CardAction.imBack(session, "buy classic white t-shirt", "Nigel Parker"),
                        builder.CardAction.imBack(session, "buy classic grey t-shirt", "Buy"),
                        builder.CardAction.imBack(session, "buy classic yellow t-shirt", "Buy")
                    ])
            ]);
            session.send(msg).endDialog();
            */
            // start the process with a group conversation

            //session.send("Let's give some Kudos");
            //builder.Prompts.choice(session, "Select a user to give Kudos to:", userList);

            /*
            var selectUserCard = new teams.O365ConnectorCardActionCard(session)
            .id("cardSelUsr")
            .name("selUsr")
            .inputs([
                new teams.O365ConnectorCardMultichoiceInput(session)
                    ...
                    .style('compact')
                    .choices([
                    new teams.O365ConnectorCardMultichoiceInputChoice(session).display("Nigel Parker").value("c6bc65bb-d858-480c-9ed1-16e324f7422f"),
                    new teams.O365ConnectorCardMultichoiceInputChoice(session).display("Nigel").value("id2")
                    ])
            ])
            .actions([
                new teams.O365ConnectorCardHttpPOST(session)
            ...
            var card = new teams.O365ConnectorCard(session)
                    .themeColor("#E67A9E")
                    .title("Select a person to give Kudos to:")
                    .potentialAction([
                    selectUserCard
                    ]);
            ...
            /*
            //var channelId = "19:7b804d6d05e84dae969b92d1fcf71b4c@thread.skype"; //session.message.address.channelId;
                ...
                if (err) {
                    ...
                    result.forEach(function(element) {
                        if (element.name!=userName) {
                            session.endDialog('%s', element.email);
                            var address = {
                                channelId: 'msteams',
                                user: { id: userId },
                                channelData: {
            ...
            //drop into an individual conversation to get feedback results.
            session.endDialog('%s', "goodbye");
